...
ここで注意したいのは、「ピン」がそのまま「伝票の束」になるわけではないことです。

HSSでは、未固定のピンを、新しい接続軸として育つ前の小さな接続候補として観測します。
...
観測するのは、未固定の接続候補が独自の接続軸として維持されるのか、それとも既存の処理ピンへ刺し直され、伝票の束として処理されるのかです。
...
| 未固定の新しいピン    | 新しい接続軸として育つ前の接続候補      | 違和感、観察、試作、顧客反応    |
| 既存の処理ピン      | 承認・予算・評価へ接続する既存ルーティング  | KPI、owner、risk、phase-gate |
| 伝票の束          | 既存の処理ピンへ刺さるように整形された処理形式 | business case、稟議、説明資料 |
...
このとき、未固定の接続候補がそのまま「伝票の束」になるわけではありません。

独自の接続軸として育つ前に、既存の処理ピンへ刺し直されることで、伝票の束として処理される場合があります。
