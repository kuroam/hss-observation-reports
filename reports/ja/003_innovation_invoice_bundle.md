...
HSSでは、新しいピンを、まだ育っていない新しい接続軸、あるいは未固定の整理軸として観測します。

その周辺には、違和感、観察、試作、顧客反応、技術的可能性などの未処理の候補があります。

一方、組織内には、承認、予算、KPI、稟議、評価、risk、owner、phase-gate、責任範囲など、すでに運用されている既存の処理ピンがあります。

未処理の候補が新しいピンの周辺に集まり、独自の接続軸を育てる場合もあります。

しかし、既存の処理形式へ早く圧縮される場合、それらの未処理の候補は、新しいピンを育てる前に、既存の処理ピンへ刺さる形式へ整形され、伝票の束として処理される場合があります。

このとき、伝票の束とは、既存の処理ピンへ刺さるように整形された business case、稟議、予算項目、KPI、説明資料、承認項目などの処理形式です。

- 新しいピン: まだ育っていない新しい接続軸、未固定の整理軸、イノベーションの種
- 未処理の候補: 違和感、観察、試作、顧客反応、技術的可能性
- 既存の処理ピン: 承認、予算、KPI、稟議、評価、risk、owner、phase-gate、責任範囲などの既存ルーティング
- 伝票の束: 未処理の候補が、既存の処理ピンへ刺さるように整形された処理形式

組織内で扱うための処理形式には、次のような問いが含まれます。
...
![ピンと伝票の束](../../assets/ja/003_invoice_bundle_pin.png)

HSSで観測するのは、ピンと伝票のどちらが正しいかではありません。

観測するのは、未処理の候補が新しいピンの周辺に集まり、独自の接続軸として育つのか、それとも既存の処理ピンへ刺さる形式へ整形され、伝票の束として処理されるのかです。

その分岐によって、どの接続経路が維持され、どの接続経路が細るように見えるかを観測します。
...
| 未処理の候補       | 新しいピンの周辺に集まりうる接続材料       | 違和感、観察、試作、顧客反応    |
| 新しいピン        | まだ育っていない新しい接続軸、未固定の整理軸  | 違和感、観察、試作、顧客反応    |
| 既存の処理ピン      | 承認・予算・評価へ接続する既存ルーティング  | KPI、owner、risk、phase-gate |
| 伝票の束          | 未処理の候補が、既存の処理ピンへ刺さるように整形された処理形式 | business case、稟議、説明資料 |
...
このとき、新しいピンがそのまま「伝票の束」になるわけではありません。

新しいピンの周辺にあった未処理の候補が、独自の接続軸を育てる前に、既存の処理ピンへ刺さる形式へ整形され、伝票の束として処理される場合があります。
...
- 「伝票の束」は承認そのものではなく、未固定の接続候補が整形される処理形式として観測できる。
- HSS上の差分は、新しい pin、未処理の候補、既存の処理 pin、伝票の束を分けて見る点にある。
- これにより、何が変換され、どの経路が太くなり、どの探索経路が細る可能性があるかが見える。

## 見えなかったこと / 保留

- このレポートでは、探索的ルーティングが残る組織や消える組織の具体例までは観測していない。
- 探索経路を保つ組織と保たない組織の構造差は保留として残る。
- 承認、予算、KPI、phase-gate が悪いと言うレポートとして読むべきではない。

## 接続確認状態

- 接続確認: 未固定の接続候補は、組織が処理できる形式へ変換されうる。
- 処理形式への吸収: business case、budget、owner、KPI、risk、phase-gate は候補を吸収しうる。
- Blue residualsあり: 違和感、prototype の痕跡、customer reaction、未解決の問いが残る場合、再接続可能な痕跡が残りうる。
- 情報不足 / 保留: 具体的な組織差は、このレポートでは確認していない。
