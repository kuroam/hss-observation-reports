...
これは、HSSを証明するものではありません。

また、ドラッカーがHSSである、という主張でもありません。

KPIやMBOが悪い、という主張でもありません。

目的は、ドラッカー的な問いが、objectives、KPI、評価制度、ダッシュボードなどへ圧縮されるとき、relation structure がどのように変化して見えるかを観測することです。

## 1. 外部ソースから取れる平均化されたドラッカー像

一般に流通しているドラッカー像では、Peter Drucker は現代マネジメント、management by objectives and self-control、knowledge worker などと結びつけて語られます。

Management by Objectives は、組織内で具体的な objectives を定義し、個人目標と組織目標を同期させ、成果を基準と比較して測る管理スタイルとして説明されることがあります。

また、ドラッカー関連の自己評価資料や一般的な解説では、mission、customer、customer value、results、plan といった問いが強調されます。

さらに、Drucker Institute や Management Top 250 などの文脈では、ドラッカー由来のマネジメント原則が、customer satisfaction、employee engagement and development、innovation、social responsibility、financial strength などの測定可能な領域へ展開される例も見られます。
...
しかし、そこでは次のような変化が十分に分解されていない場合があります。
...
組織は、抽象的な目的や価値だけでは動きにくく、何らかの objectives や指標や評価制度を必要とします。

問題は、圧縮された symbol が、元の目的、顧客、価値、成果へ再接続されなくなる場合です。
...
| Self-control            | 自己管理・目標達成責任  | 自律の自己責任化リスク   |
| MBO                     | 目標同期・評価      | 目的関数の捕獲リスク    |
| Performance measurement | 達成率・報酬・評価    | 揺り戻し欠損リスク     |

## 5. HSSモデルから推測できる仮説

### 仮説1: KPIは目的の敵ではなく、目的の圧縮symbolである

KPIは、目的の敵ではありません。

KPIは、mission、customer、value、results などを実務で扱うための圧縮symbolとして機能します。

問題は、KPIが元の接続経路へ再接続されなくなる場合です。

そのとき、KPIは補助線ではなく目的そのもののように振る舞い始めます。

### 仮説2: MBOの失敗は目標そのものではなく、systemic viewへの再接続欠損で起きる

MBOの限界として、システム理解を欠いた目標設定が、品質や本来の成果から切断される可能性が指摘されることがあります。

HSSで見ると、問題は objectives そのものではなく、objectives が systemic view、quality、purpose へ戻れなくなることです。

つまり、MBOの失敗は、目標設定の失敗というより、目標が上位接続経路へ戻れなくなる失敗として観測できます。
...
ドラッカーのマネジメントがKPIへ「堕落する」のではありません。

むしろ、ドラッカー的な問いが実務運用のためにKPIや評価指標へ圧縮されます。
