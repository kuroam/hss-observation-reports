...
ベルトコンベア型消費とは、特定のアプリ利用そのものではなく、平台・検索・推薦・ランキング・自動再生・プレイリストなどによって候補列が先に供給され、利用者がその候補列の中で、見る、聴く、飛ばす、保存する、反応する、フォローする、といった選別を行う消費形式として観測できます。

ここでいうベルトコンベア構造は、TikTok、YouTube Shorts、Instagram Reels、YouTube、Spotify、Apple Music、YouTube Music などに代表される、フィード・推薦・ランキング・自動再生・プレイリスト型の候補供給構造を指します。

これらは、個別サービスの価値評価ではなく、候補供給構造の例として扱います。

この構造は、AI生成物が流入する前から存在していました。

AI生成物がベルトコンベアを作ったのではありません。

生成AIは、そのベルトコンベアへ流し込めるsymbolの量と速度を増幅します。

重要なのは、供給側の入口制約が緩く、大量流入した候補を平台・フィード・推薦・ランキング・プレイリスト・反応指標が後段で処理し、受信側には個人最適化された候補列として提示する市場構造です。

この構造では、供給側の入口で強く絞り込むのではなく、大量に流入した候補を平台側が並べ、受信側には個人最適化された候補列として提示します。

利用者は、その候補列の中で、見る、聴く、飛ばす、保存する、反応する、フォローする、といった選別を行います。

AI生成物の大量流入は、この候補列に低接続symbolを大量に混ぜ込み、これまで市場が提供していた「探さなくても候補が届く」受信環境の快適さを低下させます。

そのため、AI slopへの不満は、AI生成物そのものへの不満だけではなく、ベルトコンベア型の候補供給構造が持っていた選別の快適さが、AI生成速度によって脅かされたことへの不満としても観測できます。
...
| 再生数 | 通過痕跡の数値化 | 価値代理指標、広告在庫、収益化可能性 |
...
- 処理形式への吸収:
  symbolの接続可能性は、再生数、クリック、滞在時間、保存、反応数などの処理形式へ吸収されやすい。
...
- HSSでは、自分の接続経路を作って選ぶことと、供給された候補ストリーム内で選択することの差が見える。
