...
初見の場合は、まず README を読んだうえで、001、003、004、006、007 の順に読むと、処理形式への圧縮、未固定の接続候補、痕跡・再接続可能領域、平台型の候補供給、指数symbolへの価格化が追いやすくなります。

002 と 005 は、それぞれ symbol の高速流通、管理・最適化・責任の個人側ルーティングを補助的に読むレポートとして扱えます。

## 観測結果の扱い

各レポートの観測結果は、HSSの成功 / 失敗ではなく、接続確認状態として扱います。
...
ただし、すべてを毎回使う必要はありません。各レポートでは、観測できたものだけを記録します。
